import {
  ...
  timer,
  zip
} from "./chunk-DMMHFXGB.js";
import "./chunk-U3R6HF27.js";
import {
...
import {
  require_buffer
} from "./chunk-JH7D5CTM.js";
import "./chunk-A74XLHLL.js";
import {
  LedgerParameters,
  Transaction,
  ZswapChainState
} from "./chunk-MDBDZZO3.js";
import {
  ContractState
} from "./chunk-U7QVBLLD.js";
import {
  __toESM
  ...
  watchFragment(options) {
    ...
    const ids = fromArray.map((value) => {
      ...
      if (__DEV__) {
        ...
        if (id === void 0) {
          __DEV__ && invariant3.warn(118, actualFragmentName);
        }
      }
      return id;
    });
...
var EntityStore = class {
  ...
  merge(older, newer) {
    ...
    if (!incoming)
      return;
    invariant3(typeof dataId === "string", 104);
              ...
              if (__DEV__) {
                const checkReference = (ref) => {
                  if (this.lookup(ref.__ref) === void 0) {
                    __DEV__ && invariant3.warn(105, ref);
                    return true;
                  }
                };
                if (isReference(newValue)) {
                  ...
                  for (const value of newValue) {
                    ...
                    if (seenReference && someNonReference !== void 0) {
                      __DEV__ && invariant3.warn(106, someNonReference);
                      break;
                    }
                  }
                }
...
function keyFieldsFnFromSpecifier(specifier) {
  ...
  return info.keyFieldsFn || (info.keyFieldsFn = (object, context) => {
    ...
    const keyObject = context.keyObject = collectSpecifierPaths(specifier, (schemaKeyPath) => {
      ...
      invariant3(extracted !== void 0, 107, schemaKeyPath.join("."), object);
      return extracted;
    });
...
var Policies = class {
  ...
  setRootTypename(which, typename = which) {
    ...
    if (typename !== old) {
      invariant3(!old || old === which, 108, which);
      if (old)
        ...
        if (supertypeSet.has(supertype)) {
          if (!typenameSupertypeSet.has(supertype)) {
            if (checkingFuzzySubtypes) {
              __DEV__ && invariant3.warn(109, typename, supertype);
            }
...
function normalizeReadFieldOptions(readFieldArgs, objectOrReference, variables) {
  ...
  if (__DEV__ && options.from === void 0) {
    __DEV__ && invariant3.warn(110, stringifyForDisplay(Array.from(readFieldArgs)));
  }
  ...
  processSelectionSet({
    ...
    this.flattenFields(
      ...
      if (value !== void 0) {
        ...
        } else if (context2.extensions?.[streamInfoSymbol] && Array.isArray(incomingValue) && hasStreamDirective(field)) {
          childTree.info = {
            field,
            typename,
      ...
      !policies.getReadFunction(typename, field.name.value)) {
        invariant3.error(115, resultKeyNameFromField(field), result);
      }
    });
    try {
...
function warnAboutDataLoss(existingRef, incomingObj, storeFieldName, store) {
  ...
  __DEV__ && invariant3.warn(117, fieldName, parentType, childTypenames.length ? "either ensure all objects of type " + childTypenames.join(" and ") + " have an ID or a custom merge function, or " : "", typeDotName, Array.isArray(existing) ? [...existing] : { ...existing }, Array.isArray(incoming) ? [...incoming] : { ...incoming });
}
  ...
  */
  ...
  constructor({ queryManager, options, transformedQuery = queryManager.transform(options.query) }) {
    ...
    if (options[variablesUnknownSymbol]) {
      invariant3(fetchPolicy === "standby", 82);
  ...
  refetch(variables) {
    ...
    if (__DEV__ && variables && hasOwnProperty7.call(variables, "variables")) {
      ...
      if (!vars || !vars.some((v) => v.variable.name.value === "variables")) {
        __DEV__ && invariant3.warn(83, variables, queryDef.name?.value || queryDef);
      }
    }
    ...
    if (shouldCancelPolling()) {
      if (__DEV__) {
        if (!this.didWarnCacheOnlyPolling && pollInterval && fetchPolicy === "cache-only") {
          __DEV__ && invariant3.warn(87, getOperationName(this.query, "(anonymous)"));
...
function logMissingFieldErrors(missing) {
  if (__DEV__ && missing) {
    __DEV__ && invariant3.debug(88, missing);
  }
}
...
var QueryManager = class {
  ...
  async mutate({ mutation, variables, optimisticResponse, updateQueries, refetchQueries = [], awaitRefetchQueries = false, update: updateWithProxyFn, onQueryUpdated, fetchPolicy, errorPolicy, keepRootFields, context }) {
    ...
    if (hasClientExports) {
      if (__DEV__) {
        invariant3(this.localState, 90, getOperationName(mutation, "(anonymous)"));
      }
    ...
    if (__DEV__ && queryNamesAndQueryStrings.size) {
      queryNamesAndQueryStrings.forEach((included, nameOrQueryString) => {
        if (!included) {
          ...
          if (queryName) {
            __DEV__ && invariant3.warn(92, queryName);
          } else {
            __DEV__ && invariant3.warn(93);
          }
        }
      });
    }
  ...
  fetchObservableWithInfo(options, {
    ...
    if (this.getDocumentInfo(normalized.query).hasClientExports) {
      if (__DEV__) {
        invariant3(this.localState, 97, getOperationName(normalized.query, "(anonymous)"));
      }
  ...
  maskOperation(options) {
    ...
    if (__DEV__) {
      ...
      if (this.dataMasking && fetchPolicy === "no-cache" && !isFullyUnmaskedOperation(document2) && !this.noCacheWarningsByCause.has(cause)) {
        ...
        __DEV__ && invariant3.warn(98, getOperationName(document2, `Unnamed ${operationType ?? "operation"}`));
      }
    }
  ...
  fetchQueryByPolicy({ query, variables, fetchPolicy, errorPolicy, returnPartialData, context }, { cacheWriteBehavior, onCacheHit, queryInfo, observableQuery, exposeExtensions }) {
    ...
    const resultsFromCache = (diff, networkStatus) => {
      ...
      if (
        ...
        if (__DEV__) {
          invariant3(this.localState, 99, getOperationName(query, "(anonymous)"));
        }
        onCacheHit();
        return from(this.localState.execute({
          client: this.client,
...
var ApolloClient = class {
  ...
  */
  query = (options) => {
    ...
    if (__DEV__) {
      invariant3(options.fetchPolicy !== "cache-and-network", 72);
      invariant3(options.fetchPolicy !== "standby", 73);
      invariant3(options.query, 74);
      invariant3(options.query.kind === "Document", 75);
      invariant3(!options.returnPartialData, 76);
      invariant3(!options.pollInterval, 77);
      invariant3(!options.notifyOnNetworkStatusChange, 78);
    }
  ...
  */
  mutate = (options) => {
    ...
    if (__DEV__) {
      invariant3(optionsWithDefaults.mutation, 79);
      invariant3(optionsWithDefaults.fetchPolicy === "network-only" || optionsWithDefaults.fetchPolicy === "no-cache", 80);
    }
  ...
  refetchQueries(options) {
    ...
    result.catch((error) => {
      __DEV__ && invariant3.debug(81, error);
    });
    return result;
  }
  /**
